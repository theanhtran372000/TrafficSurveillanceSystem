...
def videos_to_frames(args, video_dir):
    vid = video_dir.split(os.path.sep)[-1].split('.')[0]
    ...
    query = "ffmpeg -i " + video_dir + " -vf fps=" + str(args.fps) + " " + os.path.join(output_dir, "%06d.jpg")
...
def extract_clips(args, frames_dir):
    # load annotations
    vid = frames_dir.split(os.path.sep)[-1]
...
if __name__ == '__main__':
    ...
    for i, video_name in enumerate(all_video_names):
        print('===== Video {} / {} ====='.format(i + 1, n_videos))
